...
/**
 * A node in the Compose tree that corresponds to a GTK [org.gnome.gtk.Widget].
 */
...
/**
 * A node in the Compose tree that corresponds to a GTK [org.gnome.gtk.Widget] that can contain other widgets.
 */
abstract class GtkComposeContainer<out W : Widget>(widget: W) : GtkComposeWidget<W>(widget) {
    ...
    final override fun addNode(index: Int, child: GtkComposeNode) {
        require(child is GtkComposeWidget<*>) {
            "io.github.compose4gtk.GtkComposeContainer only works with io.github.compose4gtk.GtkComposeWidget"
        }
        addNode(index, child)
    }
